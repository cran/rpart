...
code would be extendeded to include more splitting functions.
...
static struct {
        ...
        double (*error)();
                 ...
                 {{anovainit,   anova,   anovass,    anovapred},
                  ...
                  {usersplit_init, usersplit, usersplit_eval,
                     usersplit_pred}
                 };
...
An easier route is to add a user defined splittin rules as a set of R functions.
...
to data copies attatched to the summary function.
...
\begin{description}
  ...
  \item[continuous] if TRUE the x variabe should be treated as continuous
  \end{description}
